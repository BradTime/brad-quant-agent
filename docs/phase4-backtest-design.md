...
### 非目标（原 Phase 4 MVP 不做，后置）
- ✅ 原 MVP 不开放任意 Python；后续「策略进化 M1」仅增加受限的 `signal-v1`
  纯信号函数（无 import/属性/I/O，独立限额进程），仍不允许自定义代码直接访问
  `Context`、Broker、网络、密钥或文件。边界见 `strategy-evolution-design.md`。
...
**自研轻量事件驱动引擎**，复用模拟交易的撮合/费用/T+1 与现有数据底座，对齐前端既定契约，以**内置策略库 + 后复权 + PIT 严谨性**先跑通可信的日频回测闭环；后续已增量完成 Backtrader 调度、分钟级、参数寻优，并以受限纯信号协议启动策略沙箱，完整进化路线见独立设计文档。
