...
```
brad-quant-agent/
├── frontend/   # Next.js 16 + React 19 + shadcn/ui + ECharts
...
**Docker 冷启动（含演示数据）**：先起 Postgres + 后端，再一次性灌入代表性个股与分组自选股：

```bash
docker compose up -d postgres backend   # 或 docker compose up --build
docker compose --profile seed up seed   # 迁移 + seed_demo.py（幂等，可重复执行）
```

`seed` 服务使用 profile，不会随默认 `docker compose up` 启动。
...
## Phase 3 功能（模拟交易）
- **模拟交易** `/sim`：100 万 play-money 账户；市价/限价单、T+1、涨跌停校验、挂单冻结与 WS 私有成交回报；AI 账户复盘（SSE）。
- **接口**：`/api/v1/sim`（account / positions / orders / trades / review）。
...
- **E2E（Playwright）**：`cd frontend && npm run e2e`（CI 在 push/PR 时自动跑 ruff+pytest、Vitest、build 与 Playwright 冒烟）
- **AI 准确性回归**见 Phase 1 小节（`scripts/ai_eval.py`）

## 开发阶段
Phase 0 地基 ✅ → Phase 1 看盘 + AI 问答 ✅ → Phase 2 盘前早报 ✅ → Phase 3 模拟交易 ✅ → Phase 4 量化研究 / 回测 ✅（AI 增强：RAG ✅ / 多智能体早报+可观测 ✅）。详见 `SPEC.md`。
