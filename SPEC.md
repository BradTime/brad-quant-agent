...
- [x] **Candidate 发布流水线**：手动、显式计价的 150 题付费评测工作流；Candidate JSON 指标与通过状态可离线复算，JUnit 题目/分类/失败载荷与 JSON 强一致，报告绑定当前路由与评测实现哈希并按 commit SHA 留存 artifact
...
### 策略进化与多 Agent 决策系统（增量）

目标是覆盖「发现标的 → 次日预测 → 组合构建 → 风险控制 → 执行 → 复盘进化」完整流程，
同时提升扣费后收益、控制最大回撤与夏普、改善次日方向/收益区间预测，并量化减少冲动交易。
系统面向全 A 股日线；今日收盘后仅使用当时可见的 PIT 数据生成信号，次日开盘后执行，
支持 1/5/20 交易日持有期。首批策略覆盖趋势、动量、均值回归、多因子、事件驱动、
组合配置和 LightGBM/XGBoost 方向/分位数预测，后续策略只能通过 Champion–Challenger 晋级。

#### 不可绕过的资金与风险约束

- 自有资金上限 20 万元、融资上限 20 万元、总敞口上限 40 万元；单票不超过净资产 20%，
  单行业不超过净资产 30%，单策略不超过总风险预算 25%，高相关策略合并计量
- 单日预计损失不超过净资产 2%；回撤 15% 警告、18% 禁止开仓、20% 强制降险
- 20% 回撤时持续排队卖出不可交易仓位、卖出其他可交易仓位，并在权限具备后允许指数工具对冲；
  首期仅使用沪深 300/中证 500 ETF 降低 Beta
- 风控由确定性「风险官」执行，优先级高于所有 Agent；任何 Agent 无权修改阈值或绕过 Kill Switch
- 股票池排除 ST/*ST、停牌、退市整理期、上市不足 120 个交易日、日均成交额低于 5000 万元、
  涨跌停不可成交及 PIT/新鲜度不足标的
- 单笔成交量不超过当日成交量 1%，默认滑点 10bp；严格模拟 T+1、整手、费税、部分成交与拒单

#### Agent 组织与决策协议

- **研究员**：由策略/统计/ML 模型产生预测、证据、候选组合和订单方案，LLM 只负责研究编排与解释，
  不直接杜撰概率或收益区间
- **反证官**：先使用不同提示词、数据切片和可选异构模型独立盲审，再查看主张进行最多两轮交叉质询；
  必须寻找反例、数据泄漏、制度限制、相关性集中、替代策略和不交易理由
- **投资委员会**：确定性评分为主、LLM 综合为辅，可批准、否决、要求补证据或选择不交易；
  必须说明未采纳反方意见的原因
- **风险官**：独立确定性规则层，最终批准订单风险；具有一票否决权
- 任一情况触发严重分歧通知：方向相反、收益中位数差异 >2%、仓位差异 >净资产 10%、
  发现未来函数/陈旧数据/重大遗漏，或投资委员会置信度 <60%
- 输出必须包含涨跌概率、收益/亏损区间、双方证据、建议仓位/订单、不交易条件、止损止盈、
  最长持有期、决策评分与否决理由

#### 策略进化、验证与行为约束

- 仅允许调整参数、组合已有因子或生成新的 Challenger；禁止策略在实盘中自改代码并立即生效
- 自定义 Python 代码必须在无网络、无密钥、无任意文件访问的隔离执行环境运行，并绑定源码 hash、
  协议版本、数据版本与随机种子
- ML 每周滚动训练，使用最近 3–5 年数据；按时间切分并采用 Purged Walk-Forward + embargo，
  禁止随机切分和标签泄漏
- 晋级门槛：扣费后年化收益为正、夏普 ≥1、最大回撤 ≤20%、相对沪深 300 正超额、
  次日方向平衡准确率 ≥53%、概率校准合格、80% 收益区间实际覆盖率 75%–85%，并覆盖牛熊震荡
- Challenger 依次经过样本外滚动回测、至少 60 个交易日模拟盘、20 个交易日影子实盘和
  20 个交易日小资金自动实盘；制度、策略逻辑或程序化报备信息重大变化后重新晋级
- 用户临时改单必须填写原因；连续亏损或频繁覆盖触发冷静期。每周对比策略内交易与人工干预收益，
  量化追涨杀跌、临时改单、撤单和偏离既定策略次数

#### 权限、通知、审计与东财接入

- 普通公开问答继续禁止替用户作最终买卖决定；只有登录后的私人「策略决策室」可在完整证据和
  风险预算下生成明确订单
- 首次启用自动交易、提高资金/杠杆、修改回撤规则、解除 Kill Switch、策略首次晋级实盘均需
  密码 + TOTP/硬件密钥二次验证；总 Kill Switch 只能人工解除
- 站内/WebSocket 与飞书只在开仓、减仓、触发风控或严重分歧时通知；飞书仅发送脱敏摘要和站内链接
- 决策、论证、策略/数据/代码版本、风控、订单、券商响应和人工覆盖形成只追加审计链；
  普通明细保留一年，敏感字段加密
- 数据源采用 Tushare Pro + 可热插拔 Provider；免费源只作降级。东财实盘仅允许官方 EMT/掘金量化
  通道，禁止 Cookie/模拟点击；完成专业投资者适当性、程序化交易报备和充分仿真前，只开放模拟盘/
  影子交易。北交所、期权、期指等按券商权限独立门控

#### 八个可验收里程碑

1. [x] **策略协议与安全沙箱**：不可变 StrategyVersion、删除留痕、源码/参数 hash、
   内置/自定义统一信号协议，受限进程执行、资源/输入/输出上限、确定性输入输出和租户隔离；
   版本化回测绑定在 M2 完成
2. [x] **全 A 日线策略与真实回测**：保存版本 ID/版本/Hash/FK 回测绑定，
   唐奇安突破、截面动量、Z-Score 反转、价格量能多因子，以及默认 10bp 滑点和信号日成交量
   1% 上限；已完成不可变完整性清单的历史全 A PIT 股票池分块物化、最多 20 标的动态过滤，
...
4. [x] **四层决策链**：研究员只解释服务端 Champion/M3 权威组合证据；反证官不接收
   主张即独立盲审，随后固定两轮结构化质询；投资委员会执行确定性评分并逐项记录反方意见处置，
   风险官保留最终否决权。方向/收益/仓位/证据/低置信度严重分歧显式标记；六阶段事件以
   输入/输出/前序 Hash 链接并由运行尾 Hash + 数量锚定，PostgreSQL 拒绝审计更新/删除和租户
   归属改写；运行采用租约 + claim token 幂等恢复，最终仍为 `executionApproved=false`
5. [x] **策略决策室与通知**：私人页面回放 M4 六阶段证据、组合风险与严重分歧；
   研究/人工复核/模拟准备模式切换、接受/拒绝/降权覆盖和 Kill Switch 全部追加审计。
   模式切换、人工覆盖、解除 Kill Switch 与账户删除使用密码 + 防重放 TOTP 的一次性用途绑定
   step-up；一次性恢复码仅能重置 TOTP/删除账户，重置会撤销全部令牌。严重分歧、风险否决和
   Kill Switch 经私有 WebSocket 与仅含站内链接的脱敏飞书通知，投递和敏感 artifact 清理均
   使用持久化 claim/outbox 无限重试；决策室仍固定 `executionEnabled=false`
6. [x] **自动模拟与晋级**：validated Challenger 必须先完成 60 个连续交易日模拟，再以
   相同现金起点完成 20 日 Champion（首次为现金基线）影子比较；收盘后预测先写不可变
   Signal Commitment，次日物化后才能按开盘、T+1、整手、1% 容量、涨跌停、10bp 滑点和
   历史费税结算，禁止事后回填。承诺/观察/阶段链由独立可轮换 HMAC key 签名，晋级时独立
   重算连续性、60+20 样本、收益/回撤/夏普/方向准确率及配对单侧 95% 下界；漏承诺或质量
   破线自动降级/暂停。人工覆盖使用同一执行模型归因，毒记录逐条隔离重试
7. [ ] **东财仿真与报备准备（基础已完成，等待官方终端验收）**：已按官方文档实现
   `gm.api` adapter/终端 bridge、加密账户绑定、报备实现指纹、二次验证演练委托、幂等 outbox、
   DB 流控、租约/断线不重发、资金/持仓/委托/成交对账和不可变演练证据。官方 SDK 的
   `MODE_LIVE` 同时承载仿真/实盘，且文档没有可机器验证的账户类型字段；因此当前 bridge
   固定只读，订单状态为 `blocked_account_type`，不会调用 `order_volume`。待在东财终端取得
