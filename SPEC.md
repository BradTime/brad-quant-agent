...
- **实现进度**：Phase 0–4 + AI 增强（RAG 含 HNSW/混合检索、多智能体、MCP）+ 工程化基线**均已完成**。剩余小项：记忆 / 微调与 Phase 4 深化（分钟回测 / Backtrader）；「产品化扩展期」（RBAC / 多市场 / i18n / 真实时）按规划后置。
...
- [ ] 后续：分钟级回测、把 backtrader 预留补成真实现并与自研对拍
