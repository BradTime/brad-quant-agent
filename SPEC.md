...
1. [x] **策略协议与安全沙箱**：不可变 StrategyVersion、删除留痕、源码/参数 hash、
   ...
   版本化回测绑定在 M2 完成
2. [ ] **全 A 日线策略与真实回测（进行中）**：已完成保存版本 ID/版本/Hash/FK 回测绑定，
   唐奇安突破、截面动量、Z-Score 反转、价格量能多因子，以及默认 10bp 滑点和信号日成交量
   1% 上限；已完成单交易日全 A PIT 股票池物化和最多 20 标的动态过滤。待完成可取消、分块/
   列式加载的历史全 A 异步回测，以及追加式 PIT 资金流/财务/事件数据后，才开放事件和基本面策略
3. [ ] **预测与组合层**：ML 方向/分位数预测、校准报告、市场状态分类、风险预算和动态策略权重
